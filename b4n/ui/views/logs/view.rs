...
use b4n_kube::{ContainerRef, ContainerType, PODS};
...
impl LogsView {
    ...
    pub fn new(
        ...
        let regular = containers
            .iter()
            .filter(|c| c.kind == ContainerType::Regular)
            .collect::<Vec<_>>();
        let container = (regular.len() == 1).then(|| regular[0].container.clone()).flatten();
