...
impl CmdView {
    ...
    fn ask_close_forcibly(&mut self) -> ResponseEvent {
        if self.bridge.is_running() {
    ...
    fn new_close_dialog(&self) -> Dialog {
        let colors = &self.app_data.borrow().theme.colors;
        Dialog::new(
            format!(
                "You are about to close '{}' while it is still running. \
    ...
    fn process_event(&mut self, event: &TuiEvent) -> ResponseEvent {
        ...
        if let TuiEvent::Mouse(mouse) = event {
            if self.is_mouse_enabled {
                ...
                if let Some(bytes) = encode_mouse(mouse.kind, mouse.column, mouse.row, self.area, mouse.modifiers) {
